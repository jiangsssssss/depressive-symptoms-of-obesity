...
Sample_description_results <- data.frame(
  ...
  No_CI_Upper = numeric(),
  Yes_Percent = numeric(),
  Yes_CI_Lower = numeric(),
  Yes_CI_Upper = numeric(),
  P_Value = numeric(), 
...
for (var in continuous_vars) {
  ...
  Sample_description_results <- rbind(Sample_description_results, data.frame(
    Variable = var,
    No_Percent = no_mean_ci[1],
    No_CI_Lower = no_mean_ci[2],
    No_CI_Upper = no_mean_ci[3],
    Yes_Percent = yes_mean_ci[1],
    Yes_CI_Lower = yes_mean_ci[2],
    Yes_CI_Upper = yes_mean_ci[3],
...
for (var in categorical_vars) {
  ...
  for (level in levels) {
    ...
    Sample_description_results <- rbind(Sample_description_results, data.frame(
      ...
      No_Percent = no_percent_ci[1],
      No_CI_Lower = no_percent_ci[2],
      No_CI_Upper = no_percent_ci[3],
      Yes_Percent = yes_percent_ci[1],
      Yes_CI_Lower = yes_percent_ci[2],
      Yes_CI_Upper = yes_percent_ci[3],
...
Variables_importance_plot_of_XGBoost <- ggplot(importance,aes(y = reorder(Feature, -Gain),x = Gain)) +
  geom_bar(stat = "identity",fill = "#69b3a2", alpha = 0.8, width = 0.6) +scale_fill_viridis() +
  labs(x = "Importance Score",y = "") +coord_flip()+theme_bw()+theme(axis.text.x = element_text(angle = 90, hjust =0.9))
...
sv_importance(shap_long_hd,kind = "beeswarm")+theme_bw()
...
ROC_of_Training_set <- ggroc(trainroc,legacy.axes = TRUE,size = 1,color = "#22b3a2") +
  geom_abline(intercept = 0,slope = 1,linetype = "dashed",color = "grey") + 
...
ROC_of_Test_set <- ggroc(testroc,legacy.axes = TRUE,size = 1,color = "#69b3a2") +  
...
ggplot(df.rf.train, aes(x=pm, y=am)) +
  geom_point() +
  geom_line() +
  geom_abline(intercept=0, slope=1, color="#69b3a2", linetype="dashed")+
